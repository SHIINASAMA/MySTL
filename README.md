...
| 原型                                                         | 注释                             |
| ------------------------------------------------------------ | -------------------------------- |
| *static* **Map&lt;key,Bin\*&gt;*** calculate(**Stack&lt;HuffmanTreeNode&lt;type&gt;*** stack); | 根据堆栈数据计算并返回哈夫曼编码 |

-----

#### *AVLTreeNode&lt;key,value&gt; - AVL树节点*
...
#### *AVLTree&lt;key,value&gt; : Cloneable - AVL树*

| 原型                                   | 注释                                                         |
| -------------------------------------- | ------------------------------------------------------------ |
| AVLTree();                             | 初始化AVL树                                                  |
| **bool** put(**key** k, **value** v);  | 添加键值对，键存在时覆盖并返回false，键不存在时新建并返回true |
| **bool** get(**key** k, **value*** v); | 根据键返回值，键存在时返回true，键不存在返回false            |
| **bool** remove(**key** k);            | 根据键删除键值对，键存在时删除并返回true，键不存在则返回false |
| **void** clear();                      | 清除树中所有元素                                             |
| **int** getCount();                    | 获取树中的元素总数                                           |
| **void*** clone();                     | 返回实例副本                                                 |
...
| 原型                                                | 注释                                                      |
| --------------------------------------------------- | --------------------------------------------------------- |
| *virtual* **Iterable&lt;value&gt;*** getNext() = 0; | 获取指向下一个节点的迭代器，若不存在下一节点则返回nullptr |
